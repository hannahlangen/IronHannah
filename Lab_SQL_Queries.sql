...
-- select max(payments), loan_id from loan group by loan_id, payments limit 1
...
select account_id,type, round(sum(amount)) as total_amount from trans where account_id = 396  group by type order by type 
